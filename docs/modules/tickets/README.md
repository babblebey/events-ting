...
The Tickets module manages individual ticket instances created from ticket type purchases. Each ticket represents a unique entry to an event with its own QR code, assignment status, and check-in tracking. This module enables multi-ticket purchases where each ticket can be assigned to a different attendee, supporting scenarios like group registrations with individual attendee information.
...
- **[Events Module](../events/)**: Tickets belong to events, assignment cutoff configured at event level
- **[Registration Module](../registration/)**: Tickets are created from registration purchases  
- **[TicketType Module](../tickets/)** (legacy): Defines ticket tiers, but instances managed separately

**This module is required by:**
- **[Attendees Module](../attendees/)**: Each ticket links to an attendee record when assigned
- **Check-in Module** (future): QR code scanning for event entry
- **[Communications Module](../communications/)**: Send emails to individual ticket attendees
