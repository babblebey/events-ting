...
export const customFieldDefinitionSchema = z
  ...
  .refine(
    ...
    {
      message:
        "Options are required for select, radio, and checkbox field types",
      path: ["options"],
    },
  )
  .refine(
 ...
 * ```
 */
export function validateFieldDefinition(field: unknown): FieldValidationResult {
  try {
    ...
    if (typedField.type === "checkbox") {
      if (
        typedField.minSelections !== undefined &&
        typedField.maxSelections !== undefined
      ) {
...
function validateCheckboxFieldResponse(
  ...
  if (
    fieldDef.minSelections !== undefined &&
    value.length < fieldDef.minSelections
  ) {
    return {
      isValid: false,
  ...
  if (
    fieldDef.maxSelections !== undefined &&
    value.length > fieldDef.maxSelections
  ) {
    return {
      isValid: false,
...
export function validateCustomFieldResponses(
  ...
  for (const fieldDef of fieldDefinitions) {
    ...
    if (!defValidation.isValid) {
      result.errors.push(
        `Invalid field definition for '${fieldDef.id}': ${defValidation.error}`,
      );
  ...
  const unexpectedFields = responseFieldIds.filter(
    (id) => !definedFieldIds.has(id),
  );

  if (unexpectedFields.length > 0) {
    result.errors.push(
      `Unexpected fields in responses: ${unexpectedFields.join(", ")}`,
    );
  }
